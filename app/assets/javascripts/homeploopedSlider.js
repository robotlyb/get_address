 	...
 	jQuery.fn.extend({ 
 		loopedSlider: function(options) {
    		return this.each(function() {
				...
				var w = obj.width(); 
				var h = obj.height();
				var f = jQuery('.'+o.container1, obj).find('div.slide:first').attr('id');
                var l = jQuery('.'+o.container1, obj).find('div.slide:last').attr('id');           
                ...
                jQuery('.'+o.container1, obj).find('div').css({ 'z-index': 0, opacity: 0 });
				jQuery('.'+o.container1, obj).find('div div').attr('style','');
				
				// load first slide
				jQuery('.'+o.container1, obj).find('div:eq(0)').animate({ opacity: 1.0 }, o.fadeSpeed, function() {						
					jQuery(this).css({ 'z-index': 100 });				
					jQuery(this).addClass('current');
					if (o.autoHeight===true) {
						// gets height of new slide
						var newHeight = jQuery(this, obj).height() + o.padding;
						jQuery('.'+o.container1, obj).animate({'height': newHeight}, o.animateSpeed, o.easing);
					}
					setToActive(this);
				});		
				...
				jQuery('.'+o.pagination, obj).find('a').click(function(){
					if(u===false  && (jQuery(this).hasClass('active')===false)) {
						...
						jQuery('.'+o.container1, obj).find('div').animate({ opacity: 0 }, o.fadeSpeed, function() {					
				...
				jQuery('.'+o.navButtons, obj).find('a').click(function(){
					if(u===false) {
						...
						if ((loop==='first') && (jQuery(this).hasClass('previous'))) {
							...
							// setsup value for new slide
							var getCurrent = jQuery('.'+o.container1, obj).find('.current').attr('id');
						...
						jQuery('.'+o.container1, obj).find('.current').animate({ 'left': nextD }, o.slideSpeed, o.easing, function() {					
							jQuery(this).removeClass('current');
